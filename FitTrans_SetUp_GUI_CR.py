...
#Edited by CR
#The below function can be used to supercede calcDspacing(), genhkl(), forbidden(), allowedDiamRefs(), and findeqvs()
def allowedDiamRefs(hmin, hmax, kmin, kmax, lmin, lmax):
    generator = ReflectionGenerator(CrystalStr)
    #Calculate limits
    dmax = CrystalCell.d(1, 0, 0)
    dmin = CrystalCell.d(hmax, kmax, lmax)
    #Generate list of hkls
    hkls = generator.getHKLsUsingFilter(dmin, dmax, ReflectionConditionFilter.StructureFactor)
    #Fix to remove unwanted extra hkl reflections outside of desired range
    hklsift = [i for i in hkls if hmin<=i[0]<=hmax and kmin<=i[1]<=kmax and lmin<=i[2]<=lmax]
    #Calculate d-spacing values, and sort the array in descending order
    dValues = generator.getDValues(hklsift)
    dValues, hklsift =  zip(*sorted(zip(dValues, hklsift), reverse=True))
    output = np.array(hklsift)
    return output
...
def findeqvs(hkl):
    ...
    eqvlab[0]=1
    for i in range(1,n):
        if pg.isEquivalent([hkl[i][0],hkl[i][1],hkl[i][2]], [hkl[i-1][0],hkl[i-1][1],hkl[i-1][2]]):
            eqvlab[i]=eqvlab[i-1]
        else:
            eqvlab[i]=eqvlab[i-1]+1
    lab = int(max(eqvlab)) +1 #Need to check +1 here, old v novercounts number of equivs by one I think, have included it here for consistency
    return eqvlab, lab
...
def pkposcalc_strain(hkl, UB, setang, epsilon):
    ...
    Is this correct?    Have added -Q here to makes sure only hkls with negative Qz are counted. 
...
class FitTransV2(PythonAlgorithm):
    def category(self):
        return 'Diffraction\attenuation'
        
    def PyInit(self):
        self.declareProperty(FileProperty(name="directory",defaultValue="/SNS/SNAP/IPTS-27111/shared/", action=FileAction.Directory), doc="Enter file directory")
        self.declareProperty(name='instrument', defaultValue='SNAP', validator=StringListValidator(['SNAP', 'PEARL']), direction=Direction.Input, doc="Select the instrument you are using")
        self.declareProperty(name='anvil material', defaultValue='Diamond', validator=StringListValidator(['Diamond']), direction=Direction.Input, doc="Select the anvil material you are using")
        self.declareProperty('runNumber','51968')
        self.declareProperty('Background terms','8')
        self.declareProperty(name='Dip peakshape', defaultValue='Pseudovoigt', validator=StringListValidator(['Psuedovoigt', 'Lorentz', 'Gaussian', 'Bk2BkExpConvPS']), direction=Direction.Input, doc="Select the peakshape to fit to the dip spectrum")
    ...
    def PyExec(self):
        ...
        global pg
        global runNumber,directory,insTag
        global CrystalStr, CrystalCell
    
        anvMat = self.getProperty('anvil material').value
        if anvMat == 'Diamond':
            #Define structure and cell of Diamond once, here
            #Note origin choice 1 by default (at -4 3 m), define crystal structure of Diamond
            #Uiso(C) = 0.0038
            CrystalStr = CrystalStructure("3.56683 3.56683 3.56683", "F d -3 m", "C 0 0 0 1 0.0038")
            CrystalCell = UnitCell(3.56683, 3.56683, 3.56683, 90, 90, 90)
            pg = PointGroupFactory.createPointGroup("m-3m")
        else:
                print('Error: only diamond is currently supported')
        ...
        if insTag == 'SNAP':
            LoadInstrument(ws1, FileName="SNAP_Definition.hdf5", RewriteSpectraMap=True)
            DSmonZ = 1.94
        elif insTag == 'PEARL':
            LoadInstrument(ws1, FileName="PEARL_Definition_new_lowangle.hdf5", RewriteSpectraMap=True)
            DSmonZ = abs(ws1.getInstrument().getComponentByName('down_monitor_det4').getPos()[2])

        L1 = abs(ws1.getInstrument().getSource().getPos()[2])
        print('L1 is set as: ' + str(L1))
        initL2 = DSmonZ
        print('Initial L2 is set as: ' + str(initL2))
        mtd.remove('ws1')
        sf = 5.7e-5
        pktype_input = self.getProperty('Dip peakshape').value
        if pktype_input == 'Gaussian':
            pktype=1
        elif pktype_input == 'Lorentz':
            pktype=2
        elif pktype_input == 'Pseudovoigt':
            pktype=3
        else:
            pktype=4
...
AlgorithmFactory.subscribe(FitTransV2)
